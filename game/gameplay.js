...
function addLabels(element){
    let left_labels = element.querySelector('#left-labels')
    let top_labels = element.querySelector('#top-labels')

    top_labels.innerHTML += `<div class="label"></div>`
    
    for(let i = 0; i < gridSize; i++){
        left_labels.innerHTML += `<div class="label"><div>${String.fromCharCode(65 + i)}</div></div>`
        top_labels.innerHTML += `<div class="label"><div>${i+1}</div></div>`
    }
}
